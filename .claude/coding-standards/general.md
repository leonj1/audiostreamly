...
Log messages MUST use static strings with arguments, NOT dynamic string interpolation. This enables:
- Log aggregation (identical templates group together)
...
log.Printf("User %s logged in", userId)  // This IS the correct Go pattern

// ✅ GOOD - Static strings with arguments
logger.info("User %s logged in", user_id)           // Python
logger.info("User logged in", { userId })           // TypeScript structured
log.Info("User logged in", "user_id", userId)       // Go structured
```

See language-specific standards for detailed examples.

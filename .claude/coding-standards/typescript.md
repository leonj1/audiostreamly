...
Log messages MUST use static strings with structured data objects, NOT template literals or string concatenation. This enables:
- Structured logging aggregation (identical messages group together)
- Log analysis and pattern matching
- Security (prevents log injection attacks)

```typescript
// ❌ BAD - Template literals (dynamic strings)
logger.info(`User ${userId} logged in from ${ipAddress}`);
logger.error(`Failed to process order ${orderId}`);
console.log("User " + userId + " logged in");

// ✅ GOOD - Static strings with structured data objects
logger.info("User logged in", { userId, ipAddress });
logger.error("Failed to process order", { orderId });
logger.warn("Connection failed", { host, port, retryCount });

// ✅ GOOD - For simple console logging during development
console.log("User logged in", { userId, ipAddress });
...
```typescript
// With template literals, these are 3 DIFFERENT log entries:
logger.info(`User 123 logged in`);   // unique string
logger.info(`User 456 logged in`);   // unique string
logger.info(`User 789 logged in`);   // unique string

// With static strings, these are the SAME log entry (3 occurrences):
logger.info("User logged in", { userId: 123 });  // same template
logger.info("User logged in", { userId: 456 });  // same template
logger.info("User logged in", { userId: 789 });  // same template
```

### Error Logging

For error objects, pass them in the structured data:
...
logger.error("Operation failed", { error, context: { userId, action } });
```

...
### Static Log Strings (MANDATORY)

Log messages MUST use static strings with structured key-value pairs, NOT fmt.Sprintf or string concatenation. This enables:
- Structured logging aggregation (identical messages group together)
- Log analysis and pattern matching
- Security (prevents log injection attacks)

```go
// ❌ BAD - Dynamic strings with fmt.Sprintf
log.Printf("User %s logged in from %s", userID, ipAddress)
logger.Info(fmt.Sprintf("Failed to process order %s", orderID))
log.Println("User " + userID + " logged in")

// ✅ GOOD - Static strings with structured logging (slog, zap, zerolog)
slog.Info("User logged in", "user_id", userID, "ip", ipAddress)
logger.Info("Failed to process order", zap.String("order_id", orderID))
log.Info().Str("user_id", userID).Msg("User logged in")

// ✅ GOOD - With context fields
...
### Why Static Strings Matter

```go
// With fmt.Sprintf, these are 3 DIFFERENT log entries:
log.Printf("User %s logged in", "123")  // unique string
log.Printf("User %s logged in", "456")  // unique string
log.Printf("User %s logged in", "789")  // unique string

// With structured logging, these are the SAME log entry (3 occurrences):
slog.Info("User logged in", "user_id", "123")  // same message
slog.Info("User logged in", "user_id", "456")  // same message
slog.Info("User logged in", "user_id", "789")  // same message
```

### Error Logging

For errors, use structured error fields:

```go
// ❌ BAD
log.Printf("Error occurred: %v", err)
logger.Error(fmt.Sprintf("Failed: %s", err.Error()))

// ✅ GOOD
slog.Error("Error occurred", "error", err)
logger.Error("Operation failed", zap.Error(err), zap.String("user_id", userID))
...
Avoid `log.Printf` and `fmt.Sprintf` for log messages.

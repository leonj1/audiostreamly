...
Log messages MUST use static strings with arguments, NOT f-strings or `.format()`. This enables:
- Structured logging aggregation (identical messages group together)
- Log analysis and pattern matching
- Security (prevents log injection attacks)

```python
# ❌ BAD - Dynamic strings (f-strings, .format())
logger.info(f"User {user_id} logged in from {ip_address}")
logger.error("Failed to process order {}".format(order_id))
logger.warning("Connection to " + host + " failed")

# ✅ GOOD - Static strings with % style arguments
...
# ✅ GOOD - Static strings with extra dict (for structured logging)
...
```python
# With f-strings, these are 3 DIFFERENT log entries:
logger.info(f"User 123 logged in")   # unique string
logger.info(f"User 456 logged in")   # unique string
logger.info(f"User 789 logged in")   # unique string

# With static strings, these are the SAME log entry (3 occurrences):
logger.info("User %s logged in", 123)  # same template
logger.info("User %s logged in", 456)  # same template
logger.info("User %s logged in", 789)  # same template
```

### Exception Details

For exception logging, use `exc_info=True` instead of formatting the exception:
...
logger.exception("Error occurred")  # Automatically includes traceback
```
